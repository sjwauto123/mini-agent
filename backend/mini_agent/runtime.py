    ...
    async def _execute(self, run_id: str) -> RunResult:
        ...
        try:
            while True:
                ...
                bundle = await manager.prepare(session_id, tools, repair_prompt)
                if bundle.over_hard:
                    bundle = await manager.prepare(session_id, tools, repair_prompt, target_ratio=self.target_context_ratio)
                ...
                if mode == "json":
                    bundle.messages.insert(1, {"role": "system", "content": "请严格返回一个 JSON 对象：直接回答时使用 action=final 并填写 answer；调用工具时使用 action=tool_call 并只填写一个工具。可用工具如下：" + json.dumps(tools, ensure_ascii=False)})
                raw = await self._model_call(run_id, model, bundle.messages, tools, tool_choice, iteration=iteration)
                event = parse_response(raw, mode)
                if isinstance(event, Final):
                    assistant_seq = await self.store.add_message(session_id, run_id, "assistant", {"content": ""})
                    streamed = ""
                ...
                if isinstance(event, Invalid):
                    repairs += 1
                    await self.store.add_trace(run_id, "model.invalid", {"code": event.code})
                    if repairs > self.max_repairs:
                        raise RuntimeError("model_protocol_error")
                    repair_prompt = f"你上一次响应无效（{event.code}）。请重新返回一个有效的最终回答，或一个有效的工具调用。不要输出其他格式。"
                    continue
                repair_prompt = None
                ...
                if existing:
                    ...
                    await self.store.add_message(session_id, run_id, "assistant", {"content": event.decision_summary or None, "tool_calls": [{"id": event.call_id, "type": "function", "function": {"name": event.name, "arguments": canonical_args}}]})
