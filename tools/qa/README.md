...
`make_readme_shots.py` 属于**展示产物**而不是验收套件：它不产出断言、不写 `evidence/`，
只把 README「界面」一节的演示图写进 `docs/images/`，因此**没有**登记进 `run.py` 的 `SUITES`
（`list` 里看不到它是预期行为，不是漏登记）。

```bash
.venv/Scripts/python.exe tools/qa/make_readme_shots.py
```

同样要求服务已在 8000 运行，并会真实调用模型（3 轮），跑完自动删除临时会话。
两个取景上的坑写在脚本模块注释里，都是这套界面结构决定的：

- 页面是 `overflow: hidden`、滚动发生在右侧工作区**内部**，所以 `full_page=True`
  截到的就是视口本身 —— 想多装内容**只能把视口调高**；
- 思考面板在回答完成后会**自动折叠**（`open` 跟随 `busy`），出图前必须手动点开，
  否则读者看不到这个功能。
